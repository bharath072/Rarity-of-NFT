...
# Check if collection_id exists in the collection table
with engine.connect() as connection:
    collection_table = Table('collection', MetaData(bind=engine), autoload=True)
    query = collection_table.select().where(collection_table.c.collection_id == contract_address)
    result = connection.execute(query).fetchone()

    if result:
        print(f"Collection ID {contract_address} already exists in the collection table.")
        exit()
    else:
        # Insert collection_id into the collection table
        insert_statement1 = insert(collection_table).values(collection_id=contract_address, contract_name=c_data['ContractName'])
        connection.execute(insert_statement1)
        print(f"Collection ID {contract_address} added successfully to the 'collection' table.")
...
# Function to fetch the token IDs
...
# Function to fetch and store token metadata in the trait_table
def fetch_and_store_metadata(token_id, contract_address):
...
# Multithreading to fetch and store metadata
with ThreadPoolExecutor(max_workers=10) as executor:
    for token_id in tokens:
        executor.submit(fetch_and_store_metadata, token_id, contract_address)
